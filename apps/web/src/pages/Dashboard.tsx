...
export default function Dashboard() {
    ...
    // Calculate the actual index to display to avoid out-of-bounds without needing an effect
    const maxIndex = Math.max(0, displayCards.length - 1);
    const activeCardIndex = selectedCardIndex > maxIndex ? 0 : selectedCardIndex;
                            ...
                            <div className="flex items-center gap-3">
                                {displayCards.length > 1 && (
                                    <div className="flex items-center gap-1">
                                        <button
                                            type="button"
                                            onClick={() => setSelectedCardIndex(activeCardIndex <= 0 ? maxIndex : activeCardIndex - 1)}
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setSelectedCardIndex(activeCardIndex >= maxIndex ? 0 : activeCardIndex + 1)}
                                            className="w-7 h-7 flex items-center justify-center rounded-full bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-accent-blue dark:hover:text-accent-blue transition-all shadow-sm border border-slate-100 dark:border-slate-700"
