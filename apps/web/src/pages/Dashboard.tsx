...
export default function Dashboard() {
    ...
    // Ensure index doesn't exceed bounds after cards update
    const activeCardIndex = Math.min(selectedCardIndex, Math.max(0, displayCards.length - 1));
                            ...
                            <div className="flex items-center gap-3">
                                {displayCards.length > 1 && (
                                    <div className="flex items-center gap-1">
                                        <button
                                            type="button"
                                            onClick={() => setSelectedCardIndex((i) => (Math.min(i, Math.max(0, displayCards.length - 1)) <= 0 ? displayCards.length - 1 : Math.min(i, Math.max(0, displayCards.length - 1)) - 1))}
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setSelectedCardIndex((i) => (Math.min(i, Math.max(0, displayCards.length - 1)) >= displayCards.length - 1 ? 0 : Math.min(i, Math.max(0, displayCards.length - 1)) + 1))}
                                            className="w-7 h-7 flex items-center justify-center rounded-full bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-accent-blue dark:hover:text-accent-blue transition-all shadow-sm border border-slate-100 dark:border-slate-700"
